...
v99 ber_korchi(vector<v99>a,v99 n)
{
    v99 rgt=0,sum=0,cr_h=0;
    for(v99 lft=0;lft<n;lft++)
    {
        while(rgt<n and (sum+a[rgt]== (sum^a[rgt])))
            sum+=a[rgt++];
        cr_h+=rgt-lft;
        if(lft==rgt)rgt++;
        else sum-=a[lft];
...
{
    /// code is here->
    v99 n;cin>>n;vector<v99>a(n);
    rep0(i,0,n)cin>>a[i];
    cout<<ber_korchi(a,n)<<ses;
}
